...
`dir(string)` : 파이썬 내장함수 dir을 통해 컨테이너가 가지고 있는 메서드를 확인 가능
...
- `.extend(iterable)`
  ...
  - 리스트에 iterable의 항목을 추가함 (+= [x, y]와 같음)
...
- `.setdefault(key[, default])`
  ...
  - `dict.get()` 메서드와 비슷한 동작을 하는 메서드
    - key가 딕셔너리에 없을 경우, default값을 갖는 key를 삽입한 후 default를 반환함
